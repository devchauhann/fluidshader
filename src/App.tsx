        ...
        {isFullScreen && (
          ...
          >
            <button
              ...
              className="fixed top-8 left-1/2 -translate-x-1/2 px-6 py-3 rounded-full glass flex items-center gap-2 md:opacity-0 md:group-hover:opacity-100 opacity-100 transition-opacity hover:bg-white/20 active:scale-95"
            >
              ...
              >
                <div className="flex items-center gap-1 group cursor-pointer">
                  <div className="w-10 h-10 flex items-center justify-center transition-colors">
                    <Icon icon="solar:palette-linear" class="text-xl text-white" />
                  </div>
                  <span className="font-medium text-lg tracking-tight">FluidShader </span>
                </div>

                <nav className="hidden md:flex items-center gap-8">
                  <a href="/examples" className="text-sm font-light text-white hover:text-white transition-colors">
                    Examples
                  </a>
                </nav>
                ...
                <button
                  ...
                  className="md:hidden w-10 h-10 flex items-center justify-center hover:bg-white/10 rounded-lg transition-colors pointer-events-auto"
                >
                  <Icon icon="solar:settings-linear" class="text-xl text-white" />
                </button>
              </motion.header>
            )}
          </AnimatePresence>
              ...
              <main className="flex-1 flex flex-col items-center justify-center text-center px-6 pointer-events-auto pb-20">
                ...
                >
                  ...
                  <h1 className="text-5xl md:text-8xl font-medium tracking-tight mb-8 leading-[1.05]">
                    ...
                    <span className="text-white">own your palette</span>
                  </h1>

                  <p className="text-lg md:text-xl font-light text-white mb-12 max-w-xl leading-relaxed">
          ...
          {!isFullScreen && (
            <>
              ...
              <motion.div
                ...
                className="hidden md:flex w-[320px] h-screen overflow-y-auto sidebar-bg rounded-l-3xl p-6 pointer-events-auto z-40 flex-col gap-8 shadow-2xl flex-shrink-0"
              >
                <div className="flex items-center gap-3">
                  <Icon icon="solar:settings-linear" class="text-lg text-white" />
                  <h2 className="font-medium text-sm">Shader Editor</h2>
                </div>

                <div className="space-y-6 flex-1 overflow-y-auto">
                  <section>
                    <h3 className="text-[10px] font-bold tracking-widest text-white uppercase mb-4">Colors</h3>
                        ...
                        return (
                          <div key={name} className="flex items-center justify-between group">
                            <label className="flex items-center gap-4 cursor-pointer">
                              ...
                              <span className="text-sm font-medium text-white group-hover:text-white transition-colors capitalize">{name}</span>
                            </label>
                            <span className="text-[10px] font-mono text-white uppercase">{hex}</span>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <section>
                    <h3 className="text-[10px] font-bold tracking-widest text-white uppercase mb-6">Params</h3>
              ...
              {isPanelOpen && (
                <motion.div
                  ...
                  transition={{ type: "spring", damping: 25, stiffness: 200 }}
                  className="md:hidden fixed right-6 top-1/2 w-[320px] max-h-[85vh] overflow-y-auto sidebar-bg rounded-2xl p-6 pointer-events-auto z-50 flex flex-col gap-8 shadow-2xl scrollbar-hide"
                >
                        ...
                        {[
                          ...
                          <div key={slider.id} className="space-y-3">
                            <div className="flex items-center justify-between text-[11px]">
                              ...
                              <span className="font-mono text-white">{(params[slider.id as keyof typeof DEFAULTS] as number).toFixed(slider.dec)}</span>
                            </div>
                            <input
                              type="range"
                              min={slider.min}
